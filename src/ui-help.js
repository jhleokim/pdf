...
(() => {
  ...
  let active=null,timer=null,origin=null;
  const hide=()=>{clearTimeout(timer);if(active)active.removeAttribute('aria-describedby');active=null;tooltip.hidden=true;};
  ...
  for(const source of document.querySelectorAll('.pro-hint:not([role]),.pro-panel-head p,.pdf-save-note,.compare-details,.deskew-direction')){
    ...
    button.addEventListener('pointerenter',e=>{if(e.pointerType==='mouse')show(button);});button.addEventListener('pointerleave',hide);
    button.addEventListener('focus',()=>{if(!matchMedia('(pointer:coarse)').matches)show(button);});button.addEventListener('blur',hide);
    button.addEventListener('pointerdown',e=>{e.stopPropagation();if(e.pointerType==='touch'){origin={x:e.clientX,y:e.clientY};timer=setTimeout(()=>show(button),320);}});
  ...
  document.addEventListener('keydown',e=>{if(e.key==='Escape')hide();});window.addEventListener('resize',hide);document.addEventListener('scroll',hide,true);
})();
