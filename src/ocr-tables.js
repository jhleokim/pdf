...
(() => {
  ...
  function textForWords(words,indices){
    const sorted=[...indices].sort((a,b)=>words[a].box[1]-words[b].box[1]||words[a].box[0]-words[b].box[0]||a-b),lines=[];
    for(const index of sorted){
      const b=words[index].box,previous=lines[lines.length-1],height=b[3]-b[1];
      if(previous&&Math.min(previous.bottom,b[3])-Math.max(previous.top,b[1])>=Math.min(previous.bottom-previous.top,height)*.45){previous.indices.push(index);previous.top=Math.min(previous.top,b[1]);previous.bottom=Math.max(previous.bottom,b[3]);}
      else lines.push({top:b[1],bottom:b[3],indices:[index]});
    }
    return lines.map(line=>line.indices.sort((a,b)=>words[a].box[0]-words[b].box[0]||a-b).map((index,at,all)=>String(words[index].text??'')+(at===all.length-1?'':String(words[index].separator??' '))).join('').replace(/[ \t\r\n]+$/u,'')).join('\n');
  }
