...
const html=fs.readFileSync(path.join(root,web?'index.html':'dist/PDF-Studio-Standalone-v'+version+'.html'),'utf8').replace('<head>',`<head><meta http-equiv="Content-Security-Policy" content="connect-src ${web?"'self' ":''}blob: data:">`);
...
fs.writeFileSync(path.join(__dirname,'fixtures',web?'local-ocr-web.html':'local-ocr.html'),html.replace('</body>','<pre id="localOCRReport" style="position:fixed;inset:0;z-index:99999;background:white;color:black;padding:30px">Running</pre><script>'+code+'</script></body>'));
