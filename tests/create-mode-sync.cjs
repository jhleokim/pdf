...
fs.writeFileSync(path.join(__dirname,'fixtures',name+'-mobile.html'),'<!doctype html><meta charset="utf-8"><title>Mode sync mobile</title><style>body{margin:0}iframe{width:390px;height:844px;border:0}</style><iframe src="'+name+'.html" title="모바일 모드 전환"></iframe><output id="mobileChecks" style="white-space:pre-wrap"></output><script>addEventListener("message",e=>{if(e.origin===location.origin&&e.data.modeSyncReport)document.getElementById("mobileChecks").textContent=e.data.modeSyncReport})</script>');
