(async()=>{
 ...
 try{
  ...
  const abortCheck=checkProAbort;checkProAbort=()=>{if($('busyText').textContent.startsWith('Pro 편집 내용을 Basic'))throw Error('Injected transfer preparation failure');return abortCheck();};try{assert(await setProMode('basic')===false,'Failure switched mode');assert(pages[1]===originalPages[1]&&stampMarks.length===6&&proMode==='pro','Failure lost edits');}finally{checkProAbort=abortCheck;}
