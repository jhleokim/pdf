...
(async()=>{
 ...
 try{
  ...
  fetch=async(url,init)=>{
   ...
   posts++;assert(init.body instanceof Blob&&init.headers['Content-Type']==='image/jpeg','JPEG binary upload page '+ordinal);maxUpload=Math.max(maxUpload,init.body.size);const n=ordinal;
   if([60,90,140].includes(n)&&!retried.has(n)){
    retried.add(n);lastBody=init.body;out.textContent=lines.join('\n')+'\nInjecting retryable failure on '+n;
    return n===90?new Response(null,{status:503,headers:{'CF-Ray':'qa90-ICN'}}):Response.json({error:'Local quota fixture',code:'VISION_RATE_LIMIT',retryAfter:1},{status:429});
   }
  ...
  assert(ocrRecords.length===212,'All 212 results visible after failures at 60, 90 and 140');
  assert(ocrRecords.every((r,i)=>r.page===i+1&&r.text==='PAGE '+(i+1)),'Page mapping retained without missing or repeated results');
  assert(posts===215,'Only failed requests retried (215 requests / 212 pages)');
  assert(builds===0,'Unedited scans never reopen the complete PDF in pdf-lib');
  assert(maxUpload<=6*1024*1024,'Every prepared upload remains inside the client/server size cap');
  ...
  out.textContent=lines.join('\n')+'\nALL PASSED · '+seconds.toFixed(2)+'s rendering/encoding with mocked OCR · max JPEG '+maxUpload+' bytes';
