...
import {LlamaTokenizer} from './node_modules/@huggingface/transformers/dist/transformers.web.js';
...
async function defaultLoader(path,signal){
  const response=await fetch(new URL('./models/paddle-vl15-community/'+path,import.meta.url),{signal});
  if(!response.ok)throw Error('Paddle 모델 파일을 읽지 못했습니다: '+path);
  return new Uint8Array(await response.arrayBuffer());
}

export async function createPaddleSession({signal,onProgress,assetLoader,maxPixels=1003520,maxTokens=1536}={}){
  signal?.throwIfAborted();
  const adapter=await navigator.gpu?.requestAdapter({powerPreference:'high-performance'});
  if(!adapter)throw Error('이 시험판에는 WebGPU를 지원하는 브라우저와 그래픽 장치가 필요합니다.');
  if(adapter.limits.maxStorageBufferBindingSize<512*1024*1024)throw Error('이 장치의 WebGPU 메모리 한도가 Paddle 모델 실행에 부족합니다.');
  const embedded=globalThis.PaddleStandalone;
  if(embedded)await embedded.configureORT(ort);
  else{ort.env.wasm.wasmPaths=new URL('./node_modules/onnxruntime-web/dist/',import.meta.url).href;ort.env.wasm.numThreads=1;ort.env.wasm.proxy=false;}
  const assetSource=assetLoader||(embedded?path=>embedded.loadAsset(path):defaultLoader);
  ...
  const load=async path=>{check();const bytes=await assetSource(path,signal);check();return bytes;};
