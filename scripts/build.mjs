...
const toolPanel=read('src/pro-tools.html').replace('<!-- cloud-ocr-tools -->',standalone?'':read('src/pro-gemini-tools.html')).replace('<!-- cloud-ocr-dialog -->',standalone?'':read('src/pro-gemini-dialog.html')+'\n'+read('src/pro-vision-dialog.html')).replace('<!-- paddle-model-option -->',standalone?'':'<option value="paddle-vl15">PaddleOCR-VL-1.5 · WebGPU</option>').replace('<!-- vision-model-option -->',standalone?'':'<option value="vision">Google Vision · 클라우드</option>');
...
block('pro-runtime', ['work-progress.js','pro-tesseract-assets.js','pro-engine.js', 'pro-document.js', 'pro-stamp.js', 'pro-ocr.js', 'pro-gemini.js','pro-vision.js', 'pro-deskew.js', 'pro-pipeline.js', 'pro-result.js', 'pro-live-preview.js', 'pro-rail.js', 'pro-ui.js','pro-tools-ui.js','pro-gemini-ui.js','pro-vision-ui.js'].filter(f=>!standalone||!f.startsWith('pro-gemini')&&!f.startsWith('pro-vision')&&f!=='pro-tesseract-assets.js').map(f => `<script id="${f.replace('.js','')}">\n${read('src/' + f)}\n</script>`).join('\n'), '</body>');
if(!standalone){
