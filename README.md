...
현재 소스 버전: **v6.4.7**. 화면 우측 하단에서 확인할 수 있습니다. 이 문서는 빌드·운영 방법을 설명하며 배포 완료 기록은 아닙니다. [합동 유지보수 점검](docs/release-v6.4.7.md) · [대용량 작업 점검 및 한계](docs/release-v6.4.6.md)
...
| 배포·전달물 | `.deploy/index.html`과 런타임 자산, API Worker | `dist/PDF-Studio-Standalone-v6.4.7.html` 한 파일 |
...
모델 준비 스크립트와 모델 출처는 아래의 PP-OCRv5 설명을 참고하세요. 웹 배포에는 `scripts/build-deploy.mjs`가 생성한 `.deploy/` 전체가 필요합니다. 스탠드얼론 결과는 `dist/PDF-Studio-Standalone-v6.4.7.html`입니다. 우측 하단 버전을 누르면 라이선스와 소스 링크를 볼 수 있습니다.
