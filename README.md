...
현재 소스 버전: **v6.2.1**. 화면 우측 하단에서 확인할 수 있습니다. 이 문서는 빌드·운영 방법을 설명하며 배포 완료 기록은 아닙니다. [v6.1.1 영역 삭제·성능 개선](docs/release-v6.1.1.md)
...
| 배포·전달물 | `.deploy/index.html`과 `.deploy/ocr/`, `.deploy/privacy/` 자산, API Worker | `dist/PDF-Studio-Standalone-v6.2.1.html` 한 파일 |

## v6.2.1 Pro 기본값·도움말

Pro는 기울기·스캔 보정과 용량 최적화를 적용하지 않은 상태로 시작합니다. 텍스트 인식은 도구의 마지막에 배치하고, 물음표 도움말은 마우스를 1초 올려두면 표시합니다. [변경 및 검증 기록](docs/release-v6.2.1.md) · [Stirling과 용량 최적화 비교](docs/compression-comparison-v6.2.1.md)
...
모델 준비 스크립트와 모델 출처는 아래의 PP-OCRv5 설명을 참고하세요. 웹 배포에는 `.deploy/index.html`, `.deploy/ocr/`, `.deploy/privacy/`, `_headers`가 필요합니다. 스탠드얼론 결과는 `dist/PDF-Studio-Standalone-v6.2.1.html`입니다. 우측 하단 버전을 누르면 라이선스와 소스 링크를 볼 수 있습니다.
