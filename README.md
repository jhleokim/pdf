...
현재 소스 버전: **v6.5.2**. 화면 우측 하단에서 확인할 수 있습니다. 이 문서는 빌드·운영 방법을 설명하며 배포 완료 기록은 아닙니다. [자동 가리기 최적화 3차 검수](docs/release-v6.5.2.md) · [탐지 오류 수정](docs/release-v6.5.1.md) · [레이어 PDF 미리보기 수정](docs/release-v6.4.8.md) · [대용량 작업 점검 및 한계](docs/release-v6.4.6.md)
...
| 배포·전달물 | `.deploy/index.html`과 런타임 자산, API Worker | `dist/PDF-Studio-Standalone-v6.5.2.html` 한 파일 |

## v6.5.0 OCR 기반 개인정보 자동 탐지

Pro의 **개인정보 마스킹 → 자동 탐지**에서 전체 OCR을 먼저 실행하고, 정보 종류와 전체/부분 가리기를 선택합니다. 주민·외국인등록번호, 계좌·카드번호, 전화번호, 이메일, 이름, 주소 후보를 원본에서 확인하고 경계를 조절한 뒤 적용합니다. 기본 프리셋 2개와 개인 프리셋 최대 8개를 제공하며, 프리셋에는 문서 내용이 저장되지 않습니다. 추가 AI나 외부 탐지 API를 사용하지 않습니다. [사용 흐름·검증·한계](docs/release-v6.5.0.md)

## v6.4.8 레이어 PDF 미리보기 수정

v6.4.7의 레이어 검사로 일반 보고서의 Pro 미리보기까지 막히던 문제를 수정했습니다. 단일 원본에서 중복 없이 페이지를 꺼낼 때 원본 레이어 표시 설정과 페이지 자원을 함께 복사합니다. 숨긴 내용은 계속 숨기며 텍스트와 도형을 이미지로 바꾸지 않습니다. 레이어 문서의 병합·반복 복제·개인정보 마스킹은 기존 제한을 유지합니다. [실문서 검증 및 범위](docs/release-v6.4.8.md)

## v6.4.7 합동 유지보수 점검

편집·되돌리기, 개인정보 처리, 출력 PDF, 키보드/모바일 UI, 배포·단독 실행 호환성을 나누어 검토했습니다. 복제본 사이의 주석 전파, 스탬프 입력 유실, 저장 경쟁 조건과 미리보기 오류 후 잘못된 페이지에 가리는 문제를 수정했습니다. 숨김 레이어가 있는 특수 PDF는 표시 상태를 안전하게 유지할 수 없는 재구성 경로를 중단합니다. [수정·검증 및 제한 사항](docs/release-v6.4.7.md)

## v6.4.6 대용량 작업 안정성

불러오기는 파일별로 완전히 읽은 뒤 반영하고 중간 취소를 지원합니다. 진행 중인 마스킹을 문서 전체 시간 제한으로 종료하던 문제, OCR 반복 검사와 편집 이력·썸네일의 자원 정리를 개선했습니다. 실제 처리 범위와 브라우저 메모리·클라우드 호출 제한은 [점검 기록](docs/release-v6.4.6.md)을 참고하세요.

## v6.4.5 페이지 작업 메뉴

Basic 전체 페이지·Pro 페이지 목록에서 우클릭하거나 페이지 아래 `⋯`를 누르면 회전·복제·순서 변경·빈 페이지 삽입·선택 저장·삭제 메뉴가 열립니다. 선택한 여러 페이지는 함께 처리하며 모바일에서도 같은 메뉴를 제공합니다. [변경 및 검증 기록](docs/release-v6.4.5.md)
...
큰 이미지의 서버 CPU 한도 초과를 줄이도록 JPEG 바이너리 전송과 OCR 응답 스트리밍을 적용했습니다. 같은 화질과 텍스트·위치 검증을 유지하며, 중간 통신 오류는 완료한 결과를 유지한 채 해당 페이지만 재시도합니다. [원인 및 검증 기록](docs/release-v6.4.3.md)

## v6.2.3 스탠드얼론 마스킹·흑백 미리보기

단독 HTML의 출처가 없는 실행 환경에서 개인정보 삭제 엔진을 시작하지 못하던 오류를 수정했습니다. 엔진을 내장 자산만 사용하는 classic Worker로 묶고, 마스킹·B&W의 미리보기와 저장 및 검색 텍스트 보존을 검증했습니다. [원인 및 검증 기록](docs/release-v6.2.3.md)

## v6.2.2 Acrobat 글꼴 호환성

검색용 OCR 글꼴을 실제 TTF와 함께 포함하여 Acrobat의 `PDFStudioOCR` 경고를 해결했습니다. 텍스트 도구에 공식 하나2.0 Regular·Bold를 추가하고 나눔 2종도 원본 글꼴 전체를 포함합니다. 서체는 웹과 단독 HTML에 포함되며 선택한 글꼴만 해석합니다. [변경 및 검증 기록](docs/release-v6.2.2.md)

Pro 기본값과 1초 도움말 변경은 [v6.2.1 기록](docs/release-v6.2.1.md), 용량 최적화 비교는 [Stirling 비교](docs/compression-comparison-v6.2.1.md)를 참고하세요.
...
여러 열로 보는 목록에서는 가로·세로 문서를 같은 세로 프레임 안에 비율을 유지해 배치합니다. 카드 높이와 파일명 위치가 같아집니다. Pro 1열 목록과 Basic의 편집 화면 크게 보기에서는 각 페이지의 실제 비율을 유지합니다. Pro 목록 너비를 늘려 여러 열이 되면 프레임도 자동으로 전환됩니다.

## 개인정보 마스킹과 검색 가능한 PDF

v6.1.1은 MuPDF.js 1.28.1을 사용해 PDF 내부의 가린 글자와 이미지 픽셀을 삭제합니다. 마스킹한 페이지도 전체 이미지로 변환하지 않습니다. 남은 글자·벡터와 기존 OCR 검색층을 유지하며, 이미지뿐인 스캔에는 기존 로컬 OCR을 추가할 수 있습니다. 디지털 텍스트만 있는 페이지는 불필요한 재인식을 건너뜁니다.

마스킹은 OCR·클라우드 전송·Pro 보정 전에 적용합니다. 기존 양식과 주석은 일반 PDF 내용으로 변환한 뒤 가리며, 링크·북마크·첨부파일·원본 메타데이터·접근성 대체 텍스트를 제거하고 사용하지 않는 객체를 정리해 새 파일로 저장합니다. 겹친 벡터 도형은 객체 단위로 삭제될 수 있습니다. 대체 텍스트에 의존한 특수 글꼴의 검색 표현은 달라질 수 있으므로 결과를 확인하세요. 안전하게 해석할 수 없는 인라인 이미지와 대체 텍스트 조합은 오류로 중단하며 이미지 변환이나 원본 반환으로 우회하지 않습니다.

작업은 기기 안의 Worker에서 실행합니다. 웹은 최초 마스킹 때 같은 출처의 엔진만 내려받으며 PDF는 전송하지 않습니다. 스탠드얼론에는 엔진까지 내장합니다. [v6.1.1 성능 측정·제약과 검증 기록](docs/release-v6.1.1.md)을 참고하세요.

## 라이선스와 빌드

PDF Studio의 프로젝트 코드는 **GNU AGPL-3.0-or-later**로 제공합니다. [LICENSE](LICENSE)와 [제3자 라이선스 및 엔진 소스](THIRD-PARTY-NOTICES.md)를 확인하세요. 사용자가 넣은 문서·OCR 결과·API 키는 프로그램 소스 공개 대상에 포함되지 않습니다.
...
모델 준비 스크립트와 모델 출처는 아래의 PP-OCRv5 설명을 참고하세요. 웹 배포에는 `scripts/build-deploy.mjs`가 생성한 `.deploy/` 전체가 필요합니다. 스탠드얼론 결과는 `dist/PDF-Studio-Standalone-v6.5.2.html`입니다. 우측 하단 버전을 누르면 라이선스와 소스 링크를 볼 수 있습니다.

## OCR 모델 선택과 진행 안내

**Pro → 텍스트 인식 → 인식 모델**에서 Paddle OCR(기본), Tesseract, 웹 전용 Google Vision을 선택합니다. 선택만으로는 런타임이나 모델을 다운로드하지 않습니다. 기존 인식 결과는 모델을 바꿔도 유지하고, 실제 인식 요청에는 선택한 모델을 사용합니다. PP-OCRv5는 시험판에서 검증한 경량 엔진을 PDF Studio에 통합한 것으로, 별도 사이트에 문서를 다시 올릴 필요가 없습니다.
...
2. **고급옵션 → 인식 결과 교정**을 엽니다. 왼쪽에는 인식에 사용한 페이지와 단어 영역, 오른쪽에는 글줄로 묶인 편집 필드가 표시됩니다. Google Vision의 많은 단어를 한 단어씩 별도 줄에 나열하지 않습니다. 엔진의 줄바꿈과 단어 위치를 함께 사용하며, 다른 단이나 표의 멀리 떨어진 칸은 나누어 표시합니다.
3. 왼쪽 단어를 클릭하면 해당 글줄로 이동하고 문장 안에서 그 단어만 선택됩니다. 오른쪽 커서 위치를 바꾸면 원본의 대응 단어가 강조됩니다. **확인 필요한 줄 · 다음**으로 확신도가 낮은 줄을 순서대로 확인할 수 있습니다. 페이지 이동·확대·맞춤으로 원문과 대조할 수 있습니다.
...
교정은 **검색·복사용 숨은 텍스트**를 바꾸며 원본 스캔 이미지에 보이는 글자는 바꾸지 않습니다. 필드를 비우면 해당 글줄을 출력 텍스트에서 제외합니다. 간단한 오타 수정은 단어 좌표를 유지합니다. 문장 전체 교체처럼 단어 대응을 유지하기 어려운 큰 수정은 해당 글줄 영역으로 저장하며, 글자 위치는 추정임을 표시합니다. 텍스트 위치가 없는 결과는 TXT 내용만 수정할 수 있고, 기존 검색 텍스트가 있어 인식을 건너뛴 페이지에는 새 교정 영역을 만들지 않습니다. 텍스트는 일반 문자열로 처리하며 HTML로 실행하지 않습니다. 새로고침하면 저장하지 않은 결과와 초안은 사라집니다.
...
3. 새로고침 후 Pro → 텍스트 인식의 엔진 목록에서 Google Vision을 선택해 한 페이지를 시험합니다. 서버 키가 없으면 동의해도 실행 버튼을 활성화하지 않습니다. 연결 확인은 설정 존재 여부만 확인하며, 실제 인증·할당량은 인식 요청 시 검증됩니다.
...
- 자동 기울기 보정은 기본으로 켜져 있으며, 최대 1,200px의 분석용 렌더에서 글줄 투영을 비교해 ±7°를 감지합니다. OCR 엔진·AI·서버를 사용하지 않습니다. 기울기가 불확실하거나 세로 방향 글줄, 링크·주석·양식이 있는 페이지는 그대로 유지하고 사유를 보고합니다. 원본 이미지·텍스트·숨은 OCR에 같은 변환을 적용하며, 기본 설정은 내용 크기를 유지하고 페이지 경계를 넓혀 잘림을 방지합니다. 번호·워터마크는 보정 뒤 똑바로 배치합니다. 90° 방향 판별이나 원근·책 휘어짐 보정은 지원하지 않습니다.
- 페이지별 수동 보정: 미리보기의 하단 눈금이나 숫자 입력으로 −60°~+60°를 0.1°씩 조절합니다. 눈금은 마우스·터치로 드래그하며, 손을 떼었을 때만 PDF에 적용합니다. `원래대로`는 해당 페이지를 0°로, `자동`은 자동 측정값으로 되돌립니다. `빈 모서리 자르기`는 기본 해제이며 해당 페이지에만 적용됩니다. 선택하면 회전으로 생긴 빈 삼각 모서리를 없애도록 원본 비율로 재단하므로 가장자리 내용도 잘릴 수 있습니다. 원본 스캔 자체의 흰 여백을 찾아 제거하는 기능은 아닙니다.
- 빈 모서리 재단을 선택한 페이지는 남는 영역 안에 완전히 포함된 기존 텍스트만 보존 검증합니다. 재단 경계·영역 밖 또는 위치를 확인할 수 없는 항목은 검증 제외 수를 안내합니다. 검증할 내부 텍스트가 없는 페이지를 텍스트 보존 확인 완료로 표시하지 않습니다.
...
- Dark는 Chrome처럼 차콜 배경과 회색 패널을 사용합니다. 화면 설정 버튼에서 Studio/Paper 테마와 밝게/어둡게/시스템 밝기를 선택하며, 선택을 저장합니다.
...
- 웹의 OCR 엔진 목록에 세 번째 옵션으로 표시됩니다. 이전 Pro 버튼 연속 클릭 활성화 방식은 사용하지 않습니다.
