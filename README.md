...
현재 소스 버전: **v5.2.1**. 화면 우측 하단에서 확인할 수 있습니다. 이 문서는 빌드·운영 방법을 설명하며 배포 완료 기록은 아닙니다. [v5.2.1 마스킹·검색 텍스트 수정](docs/release-v5.2.1.md)
...
| 배포·전달물 | `.deploy/index.html`과 `.deploy/ocr/` 자산, API Worker | `dist/PDF-Studio-Standalone-v5.2.1.html` 한 파일 |

## 개인정보 마스킹과 검색 가능한 PDF

Pro에서 **가릴 영역 지정 → 텍스트 인식 → 확인한 결과를 PDF에 포함 → 저장** 순서로 사용합니다. 마스킹한 페이지는 가린 화면을 OCR에 전달하고, 해당 마스킹·페이지 설정과 일치하는 확인된 인식 결과만 검색·복사 가능한 텍스트로 넣습니다. 원본의 숨은 개인정보 텍스트는 복사하지 않습니다. 마스킹이나 페이지 보정 설정이 바뀌면 다시 인식해야 합니다.

마스킹하지 않은 페이지는 출력 페이지의 기존 검색 텍스트를 보존합니다. 마스킹한 페이지에서 OCR을 실행·확인하지 않은 경우에는 이미지로 저장되며 결과 보고에 검색 텍스트가 없는 페이지 수를 표시합니다. 양식·링크는 일반 페이지로 저장하고 첨부파일·원본 메타데이터는 제거합니다. 로컬 OCR은 문서를 외부에 전송하지 않습니다.
